...
def validate_card(card: cardInfo) -> bool:
    return (luhn_checksum(card.number) and datetime(card.exp_year, card.exp_month, 1) > datetime.now())


def main() -> None:
    card = Card(number="22338838382", exp_month=6, exp_year=8)
    alice = Customer(name="Alice", phone="72817388", card=card)
