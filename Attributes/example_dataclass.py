...
class Product:
    ...
    def __post_init__(self) -> None:
        if self.unit_price < 0:
            raise ValueError("unit_price attribute must be grater than 0")
        
        if self.shipping_weight < 0:
            raise ValueError("shipping_weight attribute must be greater than 0")
        
        if not 0 < self.tax_percent < 1:
            raise ValueError("tax_percent attribute must be between 0 and 1")
