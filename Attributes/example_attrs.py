...
def positive_number(instance: type, attribute: Attribute[str], value: int | float) -> None:
    """ Custom check wheather an attribute of an instance has a positive value assigned"""
    class_name = instance.__class__.__name__
    if value <= 0:
        raise ValueError(f"{class_name} {attribute.name} attribute must be greater than 0")
    
    
def percentage_value(instance: type, attribute: Attribute[str], value: float) -> None:
    """ Custom check wheather an attribute of an instance has a percentage assigned """
    class_name = instance.__class__.__name__
    if not 0 <= value <= 1:
        raise ValueError(f"{class_name} {attribute.name} attribute must be between 0 and 1")
...
class Product:
    ...
    unit_price: int = field(validator=[validators.instance_of(int), positive_number], eq=False)
    tax_percent: float = field(validator=[validators.instance_of(float), percentage_value], eq=False)
...
def main() -> None:
    ...
    mango = Product(name="mango", 
                    category="fruit",
                    shipping_weight=2.0,
                    unit_price=319,
                    tax_percent=0.11)
    
    expensive_mango = Product(name="Mango", 
                              category="Fruit",
                              shipping_weight=4.0,
                              unit_price=800,
                              tax_percent=0.20)
