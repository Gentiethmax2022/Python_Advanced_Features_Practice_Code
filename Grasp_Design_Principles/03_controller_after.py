...
class Model:
    def __init__(self) -> None:
        self.connection = sqlite3.connect("products2.db")
    ...
    def get_products(self) -> list[tuple[str, float]]:
        products: list[tuple[str, float]] = [(row[0], row[1]) for row in 
...
class App(tk.Tk):
    ...
    def update_products_list(self) -> None:
        ...
        for item in self.model.get_products():
            ...
            self.product_list.insert(tk.END, product)     #type: ignore 
        self.del_product_button.config(state=tk.DISABLED)
        
    def bind_add_product(self, callback: Callable[[tk.Event], None]):
        self.add_product_button.configure(command=callback)  #type: ignore
        
    def bind_del_product(self, callback: Callable[[tk.Event], None]):
        self.del_product_button.configure(command=callback)  #type: ignore 
...
class Controller:
    ...
    def add_product(self, event=None):
        ...
        price = simpledialog.askstring(title="Add", prompt="Product Price $: ")
        if price is not None:
            price = float(price)
        else:
            raise TypeError("You must enter a real number")
